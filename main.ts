...
namespace CXgoLite {
    ...
    // A follower is not programmed as it receives its
    ...
    export function isPlaying(player: Player): boolean {
    ...
    export function randomInt(min: number, max: number): number {
        ...
        o = max - min + 1
        o = min + Math.floor(Math.random() * o)
        return o
    }
